...
def parse_args():
    ...
    parser.add_argument("--random-prob", type=float, default=0.0, help="Range of random window sampling")
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            for grad_step in range(args.num_grad_steps):
                ...
                mask = (torch.rand(orig_batch['input_ids'].shape) > args.random_prob).to('cuda')
                batch['input_ids'] = orig_batch['input_ids'] * mask
