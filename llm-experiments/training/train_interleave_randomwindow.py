...
def parse_args():
    ...
    parser.add_argument("--random-window", type=int, default=0, help="Range of random window sampling")
...
def main():
    ...
    with torch.no_grad():
        for eval_step, eval_batch in enumerate(eval_dataloader):
            ...
            eval_batch['input_ids'] = eval_batch['input_ids'][:, args.block_size // 4 : 3 * args.block_size // 4]
            eval_batch['attention_mask'] = eval_batch['attention_mask'][:, args.block_size // 4 : 3 * args.block_size // 4]
            eval_batch['labels'] = eval_batch['labels'][:, args.block_size // 4 : 3 * args.block_size // 4]
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            for grad_step in range(args.num_grad_steps):
                ...
                start_idx = random.randrange(-args.random_window, args.random_window+1)
                batch['input_ids'] = orig_batch['input_ids'][:, args.block_size // 4 + start_idx : 3 * args.block_size // 4 + start_idx]
                batch['attention_mask'] = orig_batch['attention_mask'][:, args.block_size // 4 + start_idx : 3 * args.block_size // 4 + start_idx]
                batch['labels'] = orig_batch['labels'][:, args.block_size // 4 + start_idx : 3 * args.block_size // 4 + start_idx]
