...
def parse_args():
    ...
    parser.set_defaults(use_pretrained_weights=False)
...
def main():
    ...
    config.num_hidden_layers = args.num_hidden_layers # Default 16
    config.num_attention_heads = args.num_attention_heads # Default 8
    config.hidden_size = args.hidden_size # Default 2048
            ...
            for name, param in model.named_parameters():
                if 'norm' not in name and 'bias' not in name:
                    ...
                    if 'query_key_value' in name or 'dense_h_to_4h' in name or 'embed_in' in name or 'embed_out' in name:
                        param *= math.sqrt(2 / (5 * args.hidden_size)) / 0.02
                    elif 'attention.dense' in name or 'dense_4h_to_h' in name:
                        param *= 2 / args.num_hidden_layers / math.sqrt(args.hidden_size) / 0.02
